@php
    $attention = \App\Support\ReceivingAttentionSummary::counts();
    $pendingCount = (int) ($attention['pendingCount'] ?? 0);
    $leftoverCount = (int) ($attention['leftoverCount'] ?? 0);
    $returnedCount = (int) ($attention['returnedCount'] ?? 0);
        ...
        >
            <div class="flex shrink-0 items-start justify-between gap-4 border-b border-slate-100 px-6 py-5">
                <div class="min-w-0">
                    ...
                    <h2
                        id="receivingDailyReminderTitle"
                ...
                <button
                    type="button"
                    id="receivingDailyReminderClose"
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="text-sm font-semibold text-slate-900">
                                {{ $pendingCount }} pending for second count / inspection
                            </p>
                            <p class="mt-0.5 text-xs leading-5 text-slate-500">
                                Deliveries waiting in the receiving queue.
                            </p>
                        </div>
                ...
                @if ($returnedCount > 0)
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-0.5 text-xs leading-5 text-slate-500">
                                Receiving reports sent back and still need follow-up.
                            </p>
                        </div>
            ...
            <div class="flex shrink-0 flex-wrap items-center justify-end gap-2 border-t border-slate-100 bg-slate-50/60 px-6 py-4">
                <button
                    type="button"
                    id="receivingDailyReminderDismiss"
...
<script>
(function () {
    const modal = document.getElementById('receivingDailyReminderModal');
    if (!modal) {
        window.openReceivingDailyReminder = function () {};
        window.dismissReceivingDailyReminder = function () {};
        return;
    }

    const sessionKey = @json('receivingDailyReminder:session:' . (session('attention_popup_token') ?: 'default'));
    const dateKey = @json('receivingDailyReminder:date:v1:' . (Auth::id() ?: 'guest'));
    ...
    window.dismissReceivingDailyReminder = dismissForSession;

    const dismissButton = document.getElementById('receivingDailyReminderDismiss');
    const closeButton = document.getElementById('receivingDailyReminderClose');
