    ...
    <script>
        (function () {
            ...
            function placeTip(el) {
                ...
                const preferBelow = !!el.closest('.pm-queue-table, .recent-row, .panel-head, .awaiting-indicator')
                    || spaceAbove < tipRect.height + gap + 4;
        ...
        window.exportPresidentTableToWord = function (triggerOrSelector, filenameOpt) {
            let table = null;
            let filename = filenameOpt || 'president-table';

            if (typeof triggerOrSelector === 'string') {
                table = document.querySelector(triggerOrSelector);
            } else if (triggerOrSelector && triggerOrSelector.nodeType === 1) {
                const btn = triggerOrSelector;
                filename = btn.getAttribute('data-word-filename') || filename;
                const target = btn.getAttribute('data-word-target') || '';
                if (target) {
                    table = document.querySelector(target);
                }
                if (!table) {
                    const host = btn.closest('.pm-table-word-host, .pm-card, section, .queue-panel, .rounded-xl, .overflow-x-auto') || btn.parentElement;
                    table = host ? host.querySelector('table') : null;
                }
            }

            if (!table) {
                if (typeof window.showMpToast === 'function') {
                    window.showMpToast({ title: 'Export failed', message: 'No table found to export.', duration: 2500 });
                }
                return;
            }

            const clone = table.cloneNode(true);
            clone.querySelectorAll('button, .row-actions, .icon-btn, script, style').forEach(function (el) {
                el.remove();
            });
            clone.querySelectorAll('[onclick], [data-tip]').forEach(function (el) {
            ...
            const html =
                ...
                'p.meta{font-size:9pt;color:#64748b;margin:0 0 14pt;}' +
            ...
            const safeName = String(filename).replace(/[^\w\-]+/g, '_').replace(/^_+|_+$/g, '') || 'president-table';
            ...
            if (typeof window.showMpToast === 'function') {
                window.showMpToast({ title: 'Word file ready', message: 'Table downloaded as ' + safeName + '.doc', duration: 2200 });
            }
        ...
        /**
         * Position the white thumb under the active option in a .pm-seg / .pm-filter-group.
         * @param {string|HTMLElement} trackOrId
         * @param {string} activeFilter  value matching data-filter on the option
         * @param {boolean} [animate=true]
         */
        window.pmUpdateSegControl = function (trackOrId, activeFilter, animate) {
