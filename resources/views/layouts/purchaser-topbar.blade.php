        ...
        @php
            $moduleHeading = match (true) {
                ...
                request()->is('purchaser/file-maintenance*') || request()->is('purchaser/brands*') || request()->is('purchaser/uom*') || request()->is('purchaser/categories*') || request()->is('purchaser/subcategories*') => match (request('tab', request()->routeIs('purchaser.uom.*') ? 'uom' : (request()->routeIs('purchaser.categories.*') ? 'categories' : (request()->routeIs('purchaser.subcategories.*') ? 'subcategories' : 'brands')))) {
                    ...
                    default => ['Brands', 'Brand records used across purchasing.'],
                },
            ...
            try {
                ...
                $attentionTotal = (int) (\App\Support\PurchaserAttentionSummary::counts()['attentionTotal'] ?? 0);
            } catch (\Throwable $e) {
                $unreadCount = 0;
                $recentNotes = collect();
                $attentionTotal = 0;
            ...
            >
                ...
                <div class="max-h-[290px] overflow-y-auto">
                    @if ($attentionTotal > 0)
                        <button
                            type="button"
                            onclick="typeof openPurchaserDailyReminder === 'function' && openPurchaserDailyReminder()"
                        ...
                        >
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="mt-0.5 text-[11px] leading-4 text-slate-500">
                                    Open the daily summary of pending requests and workflow items.
                                </p>
                            </div>
                        </button>
                    @endif
                        ...
                        >
                            @if ($attentionTotal > 0)
                                ...
                                <p class="mt-1 text-[10px] text-slate-500">
                                    Open Attention needed today above for workflow items.
                                </p>
                            @else
                                <div
                                    class="flex h-9 w-9
