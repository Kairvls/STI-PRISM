...
@php
    ...
    $roleName = $user->role->role_name ?? 'Receiving Officer';
...
<div class="admin-page mx-auto max-w-3xl">
    ...
    <form
        ...
        method="POST"
        action="{{ route('receiving.profile.update') }}"
        ...
        <div class="grid grid-cols-1 gap-4 px-6 py-6 sm:grid-cols-2">
            ...
            <div>
                ...
                @include('partials.phone-input', [
                    ...
                    'id' => 'receiving-user-contact-number',
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (input) {
            input.addEventListener('change', async function () {
                ...
                try {
                    const response = await fetch('{{ route('receiving.profile.picture') }}', {
                        method: 'POST',
                        headers: {
        ...
        if (removeBtn) {
            removeBtn.addEventListener('click', async function () {
                ...
                try {
                    const response = await fetch('{{ route('receiving.profile.picture.remove') }}', {
                        method: 'DELETE',
                        headers: {
                            'Accept': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
