...
<div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between fade-in">
    ...
    <div class="flex items-center gap-2">
        <a href="/president/reports/approved" class="inline-flex h-10 items-center justify-center rounded-lg border border-emerald-200 bg-white px-4 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-50 active:scale-95">
            <i data-lucide="badge-check" class="h-4 w-4"></i>
            Approved
        </a>

        <a href="/president/reports/monthly-summary" class="inline-flex h-10 items-center justify-center rounded-lg bg-gray-900 px-4 text-sm font-semibold text-white transition hover:bg-gray-800 active:scale-95">
...
<div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
    ...
    <section class="lg:col-span-1 rounded-xl border border-gray-200 bg-white p-5 slide-up" style="animation-delay: 0.05s">
        <div class="flex items-start justify-between gap-3">
            <div>
                <h2 class="text-sm font-semibold text-gray-900">Filter outcomes</h2>
                <p class="mt-1 text-xs text-gray-500">Use filters to find a specific rejection.</p>
            </div>
                    ...
                    @isset($rejectedOutcomeRecords)
                        @forelse($rejectedOutcomeRecords as $row)
                            @php
                                ...
                                $rejectedAt = $row->decided_at ?? ($row->ris_created_at ?? null);
                            ...
                            >
                                ...
                                <td class="px-3 py-4 text-sm text-gray-700">
                                    {{ $rejectedAt ? (is_object($rejectedAt) ? $rejectedAt->format('Y-m-d') : date('Y-m-d', strtotime((string)$rejectedAt))) : '—' }}
                                </td>
                                <td class="px-3 py-4 text-sm text-gray-700">{{ $remarks ?: '—' }}</td>
                                <td class="px-3 py-4 text-center">
                                    <button type="button" class="action-btn inline-flex h-8 items-center justify-center rounded-lg bg-white px-3 text-xs font-semibold text-slate-700 border border-gray-200 transition-all duration-200 hover:bg-gray-50 active:scale-95" title="View rejected RIS form" onclick="openRisViewModal({{ $row->ris_id }})">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-2 py-12 text-center">
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">Rejected RIS records will appear here.</p>
                                </td>
                            </tr>
                        @endforelse
                    @else
                        <tr>
                            <td colspan="4" class="px-2 py-12 text-center">
                                ...
                                <p class="mt-1 text-xs text-gray-500">Rejected outcomes will appear here once decisions are made.</p>
                            </td>
                        </tr>
                    @endisset
            ...
            <div class="border-b border-gray-100 px-6 py-5">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p id="risViewTitle" class="mt-1 text-sm text-slate-600">Rejected RIS</p>
                    </div>
                    <div class="flex items-center gap-2">
