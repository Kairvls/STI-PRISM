...
<div class="admin-page mx-auto max-w-3xl">
    ...
    <form
        ...
        method="POST"
        action="{{ route('president.profile.update') }}"
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (input) {
            input.addEventListener('change', async function () {
                ...
                try {
                    const response = await fetch('{{ route('president.profile.picture') }}', {
                        method: 'POST',
                        headers: {
        ...
        if (removeBtn) {
            removeBtn.addEventListener('click', async function () {
                ...
                try {
                    const response = await fetch('{{ route('president.profile.picture.remove') }}', {
                        method: 'DELETE',
                        headers: {
                            'Accept': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
