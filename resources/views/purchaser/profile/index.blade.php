...
@php
    ...
    $roleName = $user->role->role_name ?? 'Purchaser';
    $profilePictureUrl = $user->profile_picture_url
        ? $user->profile_picture_url.'?v='.time()
        : null;
@endphp

<div class="admin-page mx-auto max-w-3xl">
    <div
    ...
    <form
        ...
        method="POST"
        action="{{ route('purchaser.profile.update') }}"
            ...
            <div class="flex items-center gap-4">
                <div class="relative shrink-0">
                    <button
                        ...
                        class="group relative flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-[#0025cc] text-xl font-semibold text-white ring-2 ring-white shadow-sm"
        ...
        <div class="grid grid-cols-1 gap-4 px-6 py-6 sm:grid-cols-2">
            ...
            <div>
                ...
                @include('partials.phone-input', [
                    ...
                    'id' => 'purchaser-user-contact-number',
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (input) {
            input.addEventListener('change', async function () {
                ...
                try {
                    const response = await fetch('{{ route('purchaser.profile.picture') }}', {
                        method: 'POST',
                        headers: {
        ...
        if (removeBtn) {
            removeBtn.addEventListener('click', async function () {
                ...
                try {
                    const response = await fetch('{{ route('purchaser.profile.picture.remove') }}', {
                        method: 'DELETE',
                        headers: {
                            'Accept': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
