...
@section('page-subtitle', 'Manage Brand Records')
    ...
    <div class="mb-7">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <h1 class="text-3xl font-semibold tracking-tight text-gray-950">Brands</h1>
                <p class="mt-2 max-w-2xl text-sm leading-6 text-gray-500">Add, update, and remove brand records used across purchasing.</p>
            </div>
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div class="border-b border-gray-100 px-5 py-5">
            <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                ...
                <form method="GET" action="{{ route('purchaser.brands.index') }}" class="flex flex-col gap-2 sm:flex-row">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search brands..." class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm outline-none focus:border-gray-300 focus:bg-white sm:w-64">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    @forelse($brands as $brand)
                        <tr class="transition hover:bg-gray-50/70">
                            ...
                            <td class="px-5 py-4">
                                <span class="rounded-full px-2.5 py-1 text-xs font-medium {{ $brand->brand_status === 'Active' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                                    {{ $brand->brand_status }}
                                </span>
                            </td>
                            <td class="px-5 py-4">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-5 py-12 text-center text-sm text-gray-400">No brands found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div @click.outside="openModal = null" class="w-full max-w-md rounded-xl bg-white shadow-2xl">
            <form method="POST" :action="openModal === 'create' ? @js(route('purchaser.brands.store')) : (`{{ url('/purchaser/brands') }}/${form.id}`)">
                ...
                <div class="border-b border-gray-200 px-6 py-4">
                    <h3 class="text-lg font-semibold text-gray-900" x-text="openModal === 'create' ? 'Add Brand' : 'Edit Brand'"></h3>
                </div>
                <div class="space-y-4 px-6 py-5">
                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700">Brand Name <span class="text-red-500">*</span></label>
                        <input type="text" name="brand_name" x-model="form.name" required class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm outline-none focus:border-gray-500">
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700">Status</label>
                        <select name="brand_status" x-model="form.status" class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm outline-none focus:border-gray-500">
    ...
    <div x-cloak x-show="openModal === 'delete'" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div @click.outside="openModal = null" class="w-full max-w-md rounded-xl bg-white shadow-2xl">
            <form method="POST" :action="`{{ url('/purchaser/brands') }}/${deleteTarget.id}`">
                ...
                @method('DELETE')
                <div class="border-b border-gray-200 px-6 py-4">
                    <h3 class="text-lg font-semibold text-gray-900">Delete Brand</h3>
                </div>
