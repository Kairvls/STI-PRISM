<div
    x-data="{
        openModal: {{ ($errors->any() && old('items.0.item_subcategory_name') !== null) ? "'create'" : 'null' }},
        form: { id: null, category_id: '', name: '', description: '', status: 'Active' },
        rows: [{ category_id: '', name: '', description: '', status: 'Active' }],
        deleteTarget: { id: null, name: '' },
        maxRows: 20,
        openCreate() {
            this.form = { id: null, category_id: '', name: '', description: '', status: 'Active' };
            this.rows = [{ category_id: '', name: '', description: '', status: 'Active' }];
            this.openModal = 'create';
            this.$nextTick(() => window.lucide && window.lucide.createIcons());
        },
        addRow() {
            if (this.rows.length >= this.maxRows) return;
            this.rows.push({ category_id: '', name: '', description: '', status: 'Active' });
            this.$nextTick(() => window.lucide && window.lucide.createIcons());
        },
        removeRow(index) {
...
>
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <a href="{{ route(($pp ?? 'purchaser').'.file-maintenance.index', ['tab' => 'subcategories']) }}" class="pur-stat-card group">
        ...
        <a href="{{ route(($pp ?? 'purchaser').'.file-maintenance.index', ['tab' => 'subcategories', 'status' => 'Active']) }}" class="pur-stat-card group">
            <div class="flex items-start justify-between gap-4">
                <div>
        ...
        <a href="{{ route(($pp ?? 'purchaser').'.file-maintenance.index', ['tab' => 'subcategories', 'status' => 'Inactive']) }}" class="pur-stat-card group">
            <div class="flex items-start justify-between gap-4">
                <div>
    ...
    <div class="pur-card">
        <div class="border-b border-gray-100 px-5 py-5">
            <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                ...
                <form method="GET" action="{{ route(($pp ?? 'purchaser').'.file-maintenance.index') }}" class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
                    ...
                    @if(request()->filled('search') || request()->filled('status') || request()->filled('category_id'))
                        <a href="{{ route(($pp ?? 'purchaser').'.file-maintenance.index', ['tab' => 'subcategories']) }}" class="box-border inline-flex h-9 shrink-0 items-center justify-center rounded-lg border border-gray-200 px-4 text-sm font-medium leading-none text-gray-600 transition hover:bg-gray-50">Clear</a>
                    @endif
                </form>
            </div>
        </div>
    ...
    >
        <div @click.self="openModal = null" class="flex min-h-full w-full justify-center">
            <div class="my-auto w-full overflow-hidden rounded-2xl bg-white shadow-2xl" :class="openModal === 'create' ? 'max-w-3xl' : 'max-w-lg'" role="dialog" aria-modal="true" aria-labelledby="subcategory-form-title">
                <form method="POST" :action="openModal === 'create' ? @js(route(($pp ?? 'purchaser').'.subcategories.store')) : (`{{ url('/'.($pp ?? 'purchaser').'/subcategories') }}/${form.id}`)">
                    @csrf
                    ...
                    <div class="flex items-start justify-between gap-4 border-b border-gray-100 px-5 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 id="subcategory-form-title" class="text-lg font-semibold tracking-tight text-gray-950" x-text="openModal === 'create' ? 'Add Sub Categories' : 'Edit Sub Category'"></h3>
                                <p class="mt-0.5 text-sm text-gray-500" x-text="openModal === 'create' ? 'Add one or more sub categories in a single save.' : 'Group under a parent category.'"></p>
                            </div>
                        </div>
                            ...
                            <template x-for="(row, index) in rows" :key="index">
                                <div class="rounded-xl border border-gray-200 bg-gray-50/60 p-3">
                                    <div class="mb-2 flex items-center justify-between gap-2">
                                        <p class="text-xs font-semibold uppercase tracking-wide text-gray-400" x-text="'Sub Category ' + (index + 1)"></p>
                                    ...
                                    <div class="grid gap-3 sm:grid-cols-2">
                                        <div>
                                            <label class="mb-1 block text-xs font-medium text-gray-600">Category <span class="text-red-500">*</span></label>
                                            <select :name="`items[${index}][item_category_id]`" x-model="row.category_id" required class="box-border h-10 w-full rounded-lg border border-gray-200 bg-white px-3 text-sm text-gray-800 outline-none transition focus:border-gray-300">
                                        ...
                                        <div>
                                            <label class="mb-1 block text-xs font-medium text-gray-600">Sub Category <span class="text-red-500">*</span></label>
                                            <input type="text" :name="`items[${index}][item_subcategory_name]`" x-model="row.name" required placeholder="Enter sub category name" class="box-border h-10 w-full rounded-lg border border-gray-200 bg-white px-3 text-sm text-gray-800 outline-none transition placeholder:text-gray-400 focus:border-gray-300">
                                        </div>
                                    </div>
                    ...
                    <div class="flex justify-end gap-3 border-t border-gray-100 bg-gray-50 px-5 py-4">
                        ...
                        <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-[#0025cc] px-4 py-2.5 text-[13px] font-semibold text-white shadow-sm transition hover:bg-blue-800">
                            ...
                            <span x-text="openModal === 'create' ? (rows.length > 1 ? 'Save Sub Categories' : 'Save Sub Category') : 'Update Sub Category'"></span>
                        </button>
                    </div>
                </form>
            </div>
    ...
    >
        <div @click.self="openModal = null" class="flex min-h-full w-full justify-center">
            <div class="my-auto w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="subcategory-delete-title">
                <form method="POST" :action="`{{ url('/'.($pp ?? 'purchaser').'/subcategories') }}/${deleteTarget.id}`">
                    @csrf
                    @method('DELETE')
                    <div class="flex items-start justify-between gap-4 border-b border-gray-100 px-5 py-4">
                        <div class="flex items-center gap-3">
