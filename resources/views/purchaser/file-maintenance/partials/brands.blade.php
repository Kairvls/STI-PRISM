...
>
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <a href="{{ route(($pp ?? 'purchaser').'.file-maintenance.index', ['tab' => 'brands']) }}" class="pur-stat-card group">
        ...
        <a href="{{ route(($pp ?? 'purchaser').'.file-maintenance.index', ['tab' => 'brands', 'status' => 'Active']) }}" class="pur-stat-card group">
            <div class="flex items-start justify-between gap-4">
                <div>
        ...
        <a href="{{ route(($pp ?? 'purchaser').'.file-maintenance.index', ['tab' => 'brands', 'status' => 'Inactive']) }}" class="pur-stat-card group">
            <div class="flex items-start justify-between gap-4">
                <div>
            ...
            <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                ...
                <form method="GET" action="{{ route(($pp ?? 'purchaser').'.file-maintenance.index') }}" class="flex flex-col gap-2 sm:flex-row sm:items-center">
                    ...
                    @if(request()->filled('search') || request()->filled('status'))
                        <a href="{{ route(($pp ?? 'purchaser').'.file-maintenance.index', ['tab' => 'brands']) }}" class="box-border inline-flex h-9 shrink-0 items-center justify-center rounded-lg border border-gray-200 px-4 text-sm font-medium leading-none text-gray-600 transition hover:bg-gray-50">Clear</a>
                    @endif
                </form>
            </div>
        </div>
    ...
    >
        <div @click.self="openModal = null" class="flex min-h-full w-full justify-center">
            <div class="my-auto w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="brand-delete-title">
                <form method="POST" :action="`{{ url('/'.($pp ?? 'purchaser').'/brands') }}/${deleteTarget.id}`">
                    @csrf
                    @method('DELETE')
                    <div class="flex items-start justify-between gap-4 border-b border-gray-100 px-5 py-4">
                        <div class="flex items-center gap-3">
                    ...
                    <div class="flex justify-end gap-3 border-t border-gray-100 bg-gray-50 px-5 py-4">
                        <button type="button" @click="openModal = null" class="rounded-lg px-3 py-2 text-sm font-medium text-gray-600 transition hover:text-gray-950">Cancel</button>
                        <button type="submit" class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-[13px] font-semibold text-red-700 transition hover:bg-slate-50">
                            Yes, delete
                        </button>
                    </div>
                </form>
