...
@section ("content")
    <div class="space-y-6">
        {{-- TRANSFER DASHBOARD --}}
        @php
            $transferMonthlyHint = $transferMonthlyPercentage === null
                ? 'New activity vs last month'
                : (($transferMonthlyPercentage > 0 ? '+' : '') . number_format($transferMonthlyPercentage, 2) . '% vs last month');
        @endphp
        <div class="mb-6 mt-6">
            @include('layouts.partials.maintenance-stat-cards', [
                'cards' => [
                    ['label' => 'Total Transfer Records', 'hint' => $transferMonthlyHint, 'value' => number_format($totalTransferRecords)],
                    ['label' => 'Transferred This Month', 'hint' => $transferMonthlyHint, 'value' => number_format($currentMonthTransfers)],
                    ['label' => 'Equipment Transferred', 'hint' => number_format($equipmentTransferredPercentage, 2) . '% of all equipment', 'value' => number_format($equipmentTransferred)],
                ],
            ])
        </div>
